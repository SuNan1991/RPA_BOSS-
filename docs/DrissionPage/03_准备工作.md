...
来源: https://drissionpage.cn/get_start/before_start/

开始前，我们先设置一下浏览器路径。

**注意**: 如果只使用收发数据包功能，无需任何准备工作。

程序默认控制 Chrome，如果要使用 Edge 或其它 Chromium 内核浏览器，设置方法是一样的。

**建议**: 尽量使用版本号在 100 以上的浏览器，旧版有些功能不支持。
...
1. 打开浏览器
2. 在地址栏输入 `chrome://version`（Edge 输入 `edge://version`）
3. 红框中显示的路径就是要获取的路径
...
如果是想临时切换浏览器路径以尝试运行和操作是否正常，可以去掉 `.save()`：
