...
来源: https://drissionpage.cn/browser_control/tabs
...
- 禁用单例模式后，一个标签页也可以被多个 Tab 对象同时控制
...
**说明**: 如 `Settings.singleton_tab_obj` 为 `True`，此属性返回标签页对象的 tab id。
...
**注意**: Tab 对象默认为单例，即一个实体标签页只有一个 `MixTab` 对象。`get_tab()` 返回的标签页可能是同一个。
...
**说明**: 当传入 `url` 参数时，程序会根据 `load_mode` 设置访问页面，除了 `none` 模式，都将等待页面加载完毕。
...
**中键点击**: 元素对象的 `click.middle()` 方法可用中键点击 `<a>` 元素，可强制在新标签页打开链接，并返回新标签页对象。
...
示例：在一个标签页中遍历列表元素，点击打开新标签页，获取信息后关闭。
