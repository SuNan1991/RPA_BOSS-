# DrissionPage 官方文档 - 浏览器控制概述

来源: https://drissionpage.cn/browser_control/intro
...
元素对象 `ChromiumElement` 是交互的执行者，如点击、文本输入、获取元素信息等。
...
1. **浏览器对象 (Chromium)**: 管理浏览器整体操作
2. **标签页对象 (Tab)**: 控制单个标签页的大部分操作
3. **元素对象 (ChromiumElement)**: 执行具体的元素交互

**操作流程**:
```
创建浏览器 → 获取标签页 → 访问网址 → 查找元素 → 元素交互
```

---

*文档获取时间: 2026-02-07*
*官方文档地址: https://drissionpage.cn/browser_control/intro*
